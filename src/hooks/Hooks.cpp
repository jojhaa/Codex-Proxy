...
using Hooks::ShouldSkipChromiumSubProcessW;
using Hooks::ShouldSkipChromiumSubProcessA;
...
// UDP 绕过时不能把此前名称查询分配的 FakeIP 发到真实网络。
static bool ResolveUdpDirectAddress(const sockaddr*& address, int& length, sockaddr_storage& resolved) {
    if (!address) return true;
    std::string host; uint16_t port = 0;
    if (!ResolveOriginalTarget(address, &host, &port)) return false;
    bool wasFake = false; int resolvedLength = 0;
    if (TryResolveDirectTargetFromFakeIp(address, host, port, &resolved, &resolvedLength, &wasFake)) {
        address = reinterpret_cast<sockaddr*>(&resolved); length = resolvedLength; return true;
    }
    if (wasFake) { WSASetLastError(WSAHOST_NOT_FOUND); return false; }
    return true;
}
...
int PerformProxyConnect(SOCKET s, const struct sockaddr* name, int namelen, bool isWsa) {
    ...
    if (config.proxy.port != 0) {
        ...
        if (soType != SOCK_STREAM) {
            ...
            if (soType == SOCK_DGRAM) {
                ...
                // 有效策略为 direct：保持直连
                sockaddr_storage directAddress{};
                if (!ResolveUdpDirectAddress(name, namelen, directAddress)) return SOCKET_ERROR;
                return isWsa ? fpWSAConnect(s, name, namelen, NULL, NULL, NULL, NULL) : fpConnect(s, name, namelen);
            }
    ...
    // ROUTE-2: 端口白名单过滤
    if (!config.rules.IsPortAllowed(originalPort) && !(originalPort == 53 && config.rules.dns_mode == "proxy")) {
        if (ShouldLogRouteDecisionInfo()) {
    ...
    if (config.proxy.port != 0) {
        ...
        // 保留原始非阻塞契约。原 socket 的 FD_CONNECT 已由 Winsock 置位，
        // select/握手不会消费此事件；调用方仍须通过原事件查询连接完成。
        // 返回 0 会触发 Chromium 对非阻塞 connect 同步成功的 NOTREACHED。
        if (pendingConnect) {
...
int WSAAPI DetourGetAddrInfo(PCSTR pNodeName, PCSTR pServiceName,
    ...
    if (pHints && pHints->ai_socktype == SOCK_DGRAM && GetEffectiveUdpAction(config) == Core::UdpAction::Direct)
        return fpGetAddrInfo(pNodeName, pServiceName, pHints, ppResult);

    // 如果启用了 FakeIP 且有域名请求
    if (pNodeName && config.fakeIp.enabled) {
        std::string node = pNodeName;
        if (!Core::UsesRemoteDns(node, config.proxy.remote_dns_domains))
            return fpGetAddrInfo(pNodeName, pServiceName, pHints, ppResult);
        std::string routeAction;
        std::string routeRule;
...
int WSAAPI DetourGetAddrInfoW(PCWSTR pNodeName, PCWSTR pServiceName,
    ...
    if (pNodeName && config.fakeIp.enabled) {
        ...
        if (!Core::UsesRemoteDns(nodeUtf8, config.proxy.remote_dns_domains))
            return fpGetAddrInfoW(pNodeName, pServiceName, pHints, ppResult);
        std::string routeAction;
        std::string routeRule;
...
struct hostent* WSAAPI DetourGetHostByName(const char* name) {
    ...
    if (name && config.fakeIp.enabled) {
        ...
        if (!Core::UsesRemoteDns(node, config.proxy.remote_dns_domains)) return fpGetHostByName(name);
        std::string routeAction;
        std::string routeRule;
    ...
    if (config.proxy.port != 0) {
        ...
        if (soType != SOCK_STREAM) {
            ...
            if (soType == SOCK_DGRAM) {
                ...
                // 有效策略为 direct：保持直连
                sockaddr_storage directAddress{};
                if (!ResolveUdpDirectAddress(name, namelen, directAddress)) return FALSE;
                return originalConnectEx(s, name, namelen, lpSendBuffer, dwSendDataLength, lpdwBytesSent, lpOverlapped);
            }
...
BOOL WINAPI DetourCreateProcessW(
    ...
    bool needInject = config.childInjection;
    ...
    if (result && needInject && lpProcessInformation) {
        ...
        const bool isChromiumSandboxSkip = ShouldSkipChromiumSubProcessW(appName, lpCommandLine);
        const bool excluded = config.IsChildInjectionExcluded(appName) || isChromiumSandboxSkip;
        ...
        if (!shouldInject) {
            ...
            if (!dllPath.empty()) {
                ...
                if (injected) {
                    ...
                    CloseHandle(lpProcessInformation->hThread); CloseHandle(lpProcessInformation->hProcess);
                    *lpProcessInformation = {}; SetLastError(ERROR_DLL_INIT_FAILED); return FALSE;
                }
            } else {
...
BOOL WINAPI DetourCreateProcessA(
    ...
    if (result && needInject && lpProcessInformation) {
        ...
        const bool isChromiumSandboxSkip = ShouldSkipChromiumSubProcessA(appName, lpCommandLine);
        const bool excluded = config.IsChildInjectionExcluded(appName) || isChromiumSandboxSkip;
        const bool compatInject = (!excluded) && ShouldAutoInjectLanguageServerNodeChild(config, appName);
        const bool shouldInject = (!excluded) && (config.childInjectionMode == "inherit" || config.ShouldInject(appName) || compatInject);
...
int WSAAPI DetourSendTo(SOCKET s, const char* buf, int len, int flags, const struct sockaddr* to, int tolen) {
    ...
    if (config.proxy.port != 0) {
        ...
        if (TryGetSocketType(s, &soType) && soType == SOCK_DGRAM) {
            ...
            const Core::UdpAction udpAction = GetEffectiveUdpAction(config);
            if (udpAction == Core::UdpAction::Direct) {
                sockaddr_storage directAddress{};
                if (!ResolveUdpDirectAddress(to, tolen, directAddress)) return SOCKET_ERROR;
                return fpSendTo(s, buf, len, flags, to, tolen);
            }
            if (udpAction == Core::UdpAction::Block) {
                uint16_t dstPort = 0;
...
int WSAAPI DetourWSASendTo(
    ...
    if (config.proxy.port != 0) {
        ...
        if (TryGetSocketType(s, &soType) && soType == SOCK_DGRAM) {
            ...
            if (udpAction == Core::UdpAction::Direct) {
                sockaddr_storage directAddress{};
                if (!ResolveUdpDirectAddress(lpTo, iToLen, directAddress)) return SOCKET_ERROR;
...
namespace Hooks {
    bool Install(bool enableNetworkHooks) {
        ...
        return fpCreateProcessW && fpCreateProcessA && (!enableNetworkHooks ||
            (fpConnect && fpWSAConnect && fpGetAddrInfo && fpGetAddrInfoW && fpGetHostByName &&
             fpSend && fpRecv && fpWSASend && fpWSARecv && fpSendTo && fpWSASendTo));
    }
