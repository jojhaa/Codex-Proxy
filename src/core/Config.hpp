...
#include "RemoteDnsPolicy.hpp"

namespace Core {
    struct ProxyConfig {
        std::vector<std::string> remote_dns_domains = DefaultRemoteDnsDomains();
    ...
    struct ProxyRules {
        ...
        // UDP 处理策略
        // "block"  - 阻断 UDP，强制应用尝试回退 TCP (针对 ChatGPT/WebView2 开启，防止 QUIC 超时重连)
        // "auto"   - SOCKS5 自动走 UDP Associate；其它代理按 udp_fallback 处理
        // "direct" - UDP 直连（保持现状）
    ...
    public:
        ...
        std::string uiLoadNotify = "none";    // 加载提示策略：none 关闭弹窗，避免沙盒进程异常
        ...
        std::vector<std::string> targetProcesses = {"ChatGPT.exe", "codex.exe", "codex-code-mode-host.exe", "codex-command-runner.exe", "msedgewebview2.exe"}; // 目标进程列表 (空=全部)
        ...
        bool ShouldInject(const std::string& processName) const {
            ...
            for (const auto& target : targetProcesses) {
                ...
                if (lowerTarget.find('.') == std::string::npos && lowerName == lowerTarget + ".exe") return true;
            }
            return false;
        }
        ...
        bool Load(const std::string& path = "config.json") {
            try {
                ...
                if (j.contains("proxy")) {
                    ...
                    if (p.contains("remote_dns_domains")) {
                        proxy.remote_dns_domains.clear();
                        if (p["remote_dns_domains"].is_array()) {
                            for (const auto& domain : p["remote_dns_domains"])
                                if (domain.is_string()) proxy.remote_dns_domains.push_back(domain.get<std::string>());
                        }
                    }
                }
                ...
                // 按域名分配 FakeIP；原始 DNS 数据包不含本层可靠的目标域名，禁止整体转发 53 端口。
                if (rules.dns_mode == "proxy") {
                    Logger::Warn("域名名单模式：原始 DNS 数据包保持本地路径，不整体代理 53 端口。");
                    rules.dns_mode = "direct";
                }
                ...
                if (j.contains("child_injection")) {
                    if (j["child_injection"].is_boolean()) {
                        childInjection = j["child_injection"].get<bool>();
                    } else if (j["child_injection"].is_object()) {
                        const auto& cj = j["child_injection"];
                        childInjection = cj.value("enabled", true);
                        if (cj.contains("child_injection_mode") && cj["child_injection_mode"].is_string()) {
                            childInjectionMode = cj["child_injection_mode"].get<std::string>();
                        }
                        if (cj.contains("child_injection_exclude") && cj["child_injection_exclude"].is_array()) {
                            childInjectionExclude.clear();
                            for (const auto& item : cj["child_injection_exclude"]) {
                                if (item.is_string()) childInjectionExclude.push_back(item.get<std::string>());
                            }
                        }
                        if (cj.contains("target_processes") && cj["target_processes"].is_array()) {
                            targetProcesses.clear();
                            for (const auto& item : cj["target_processes"]) {
                                if (item.is_string()) targetProcesses.push_back(item.get<std::string>());
                            }
                        }
                    }
                } else {
                ...
                // 顶层子进程注入模式（如果存在则覆盖）
                if (j.contains("child_injection_mode") && j["child_injection_mode"].is_string()) {
                    childInjectionMode = j["child_injection_mode"].get<std::string>();
                }
